...
public class NSGA3Util {
    ...
    private static void recursiveReferencePoints(List<MOOPUtils.ReferencePoint> points, MOOPUtils.ReferencePoint point,
    ...
    public static void generateReferencePoints(List<MOOPUtils.ReferencePoint> points, int numberOfObjectives, List<Integer> p) {
        ...
        if (p.size() > 1) {
            ...
            for (MOOPUtils.ReferencePoint anInside : inside) {
                for (int j = 0; j < anInside.location.length; j++) {
                    anInside.location[j] = (center + anInside.location[j]) / 2;
                }
            }
